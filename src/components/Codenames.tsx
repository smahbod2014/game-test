...
function GetRedRemaining(game: Game): number {
  return game.RedIndexes.length - game.RedIndexes.filter((value) => game.RevealedIndexes.includes(value)).length;
}

function GetBlueRemaining(game: Game): number {
  return game.BlueIndexes.length - game.BlueIndexes.filter((value) => game.RevealedIndexes.includes(value)).length;
}
  ...
  return (
    <Box bg="#282c34" minH="100vh">
      ...
      <Box w="1000px" ml="auto" mr="auto">
        <Box mt-30 flex justifyContent="space-between">
          <Box white text3XL>
            <Box inline color={gameState.StartingTeam}>
              {gameState.StartingTeam === "red" ? GetRedRemaining(gameState) : GetBlueRemaining(gameState)}
            </Box>
            <Box inline>&#160;-&#160;</Box>
            <Box inline color={gameState.StartingTeam === "red" ? "blue" : "red"}>
              {gameState.StartingTeam === "red" ? GetBlueRemaining(gameState) : GetRedRemaining(gameState)}
            </Box>
          </Box>
        ...
        <Box flex flexWrap="wrap" h="670px" mt-20 justifyContent="space-between">
          {gameState.Words.map((word, index) => {
            ...
            return (
              <Box
                ...
                cursorPointer={!isSpymaster && !gameState.GameOver && !IsAlreadyRevealed(gameState, index)}
                onClick={() => {
                  if (isSpymaster || gameState.GameOver) {
                    return;
                  }
                  socket.emit("selection", JSON.stringify({ Index: index, GameID: gameID }));
                }}
              >
                {word}
              </Box>
        ...
        <Box flex justifyContent="flex-end" mt="10px" color="white" alignItems="center">
          <div className="form-check form-switch">
            ...
            />
            <label className="form-check-label hoverable" htmlFor="flexSwitchCheckChecked">
              Spymaster
            </label>
          </div>
          <Box
